...
export default class QuadraticBezierInterpolation {
    ...
    constructor(p0, p1, p2) {
        ...
        if (4 * p * p * p + 27 * q * q >= 0) {
            ...
            // three real roots
            for (let k = 0; k < 3; k++) {
                const t = 2 * Math.sqrt(-p / 3) * Math.cos(1. / 3 * Math.acos(3 * q / 2. / p * Math.sqrt(-3 / p)) - 2 * Math.PI * k / 3.) - b / 3 / a;
                if (0 <= t && t <= 1) {
                    ti = t;
                }
            }
        ...
        const b1 = p1.clone().sub(p0.clone().multiplyScalar(square(1 - ti))).sub(p2.clone().multiplyScalar(square(ti))).divideScalar(2 * (1 - ti) * ti);
